...
| **合计** | **96** | 14/14 需求全覆盖；**负向 51 / 正向 45**（负向=预期 VCU 报错/拒绝/卡死/维持）|
...
把工具实际使用的 **5 个 prompt** 完整粘贴到 `docs/test_evidence/prompts_used.md`（不能只写“见代码”）：`parse`（需求解析）、`risk`（风险分析）、`coverage`（覆盖项）、`testcase`（用例+oracle 生成）、`improve`（两轮模糊改进）。每个配真实“输入→输出”样例。
...
4. **三联追溯**：`docs/test_evidence/10_traceability_matrix.csv`（96 行，列含 Test Case ID / Coverage Item ID / Requirement ID / Technique / RPN / **Polarity 正负向**）。
...
| T5 | state11 | 连续 ≥3 次快速(间隔<1s)唤醒-休眠循环 | state10（卡死）| SC 卡死用例（REQ-012）|
...
- RPN ≤ 5（High，对应 REQ-001~012）→ **100% 必过** —— 已达成
- RPN 6~10（Medium，REQ-013/014）→ ≥ 95% —— 已达成

**通过 ≠ 仿真器全 success**：96 条里 **51 条负向**（预期 VCU 报错/拒绝/卡死/维持），它们通过=确认 VCU 正确报错。若有用例失败且非 oracle 缺陷，则 §10.3 写缺陷报告（不硬改用例）。本轮 oracle 缺陷均已在 REV-016 执行期审查修正。
...
REQ ↔ 用例追溯来自 `10_traceability_matrix.csv`（14/14 需求全覆盖，96 条全 PASS）：

| REQ | 技术 | 用例数 | Result | Status |
|---|---|---|---|---|
| REQ-001~007（7 路唤醒）| EP+BVA+ST | 53 | 全 PASS | Covered |
| REQ-008~011（休眠条件）| EP+DT+ST | 24 | 全 PASS | Covered |
| REQ-012（卡死缺陷）| SC+ST | 6 | 复现 stuck（state10+DTC_001）| Covered（**DEF-001 已确认**）|
| REQ-013（输出一致性）| 多技术 | 7 | 全 PASS | Covered |
| REQ-014（时序合规）| BVA | 6 | actual_duration 正常 ≤20/60；违规由卡死 41s 体现 | Covered |

**11.2 Coverage Item Coverage**：22 个 Module A Coverage Item，技术覆盖 EP/BVA/DT/ST/SC 五种齐全（黑盒 EP/BVA/DT/SC + 白盒 ST），每条 CI 均有用例覆盖。
