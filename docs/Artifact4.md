...
**Out of Scope**：真实 HIL/dSPACE、ASAM XCP 物理通信、CAN 物理层、1kHz 物理采样、真车路测；以及 **Module B/C/D/E**（过压/欠压保护、CAN 错误、DTC 清除、功耗告警等）。
...
→ 结果分析与改进
```
本项目共记录了 16 项人工修订（REV-001~REV-016） 和 8 项工具修复（TOOL-FIX-001~TOOL-FIX-008）。这些修订覆盖了需求解析、风险评分、覆盖项补全、测试策略调整、测试用例审查、执行后纠正和基于证据的改进等阶段。关键修订摘要如下:
...
AutoTestDesign 首先通过 `parse` prompt（§6.2）将 §2.1 中的 14 条 Module A 需求结构化为 JSON 需求对象（`01_parsed_requirements_raw.json`）。测试人员随后对照需求说明和 VCU Simulator 的可观察输入/输出字段进行审查，发现若干字段、时序条件和 oracle 表达问题，并在进入后续测试设计前完成修订：
...
> 下表为经 §2.2 的 REV-001~005 修订后的最终受测规则（非 LLM 原始解析输出）。
...
14 条结构化需求经 `risk` prompt（§6.2）进行 ISO 9126 × (Tech Risk × Business Risk) 风险评分，初始结果保存在 `05_risk_analysis_raw.json`。人工复核后确认大部分评分可作为测试设计输入，其中 REQ-012 卡死缺陷被评为 RPN=1，符合其已知缺陷和状态机失效影响。复核过程中发现 REQ-008 和 REQ-009 的风险被低估，因此对这两项进行人工修订：
...
**最终 RPN 分布**：Extensive（RPN 1~5）7 条，Broad（RPN 6~10）7 条。其中 RPN≤2 的重点需求为 REQ-001、REQ-008、REQ-009、REQ-010、REQ-011 和 REQ-012。该 RPN 用于驱动 §8 追溯矩阵中的优先级列、§10 的执行顺序和 §10.5 的测试套件优先级排序。
...
> Coverage Item 在 AutoTestDesign 的 Coverage 页维护（技术归属见 §5 Strategy 总览）。初始覆盖项主要集中在输入条件，人工审查后补充 Output、Behavior 和 Environment 三类覆盖目标，使 Coverage Item 从 14 条扩展到 22 条（REV-006~008）。
...
**分析**：初始 14 条覆盖项主要关注输入边界，尚不足以支撑可执行 oracle、状态序列和环境前提的完整测试设计。人工评审据此补充 **8 条覆盖项**：① **5 条 Output**（vehicle_state、pdcu_wake_reason、bus_message_flag 与状态一致性、actual_duration、active_dtcs 包含 DTC_001），用于明确通过/失败判定；② **2 条 Behavior**（正常唤醒序列、卡死序列），用于支撑白盒状态迁移测试和场景测试；③ **1 条 Environment**（SIL 环境基线），用于说明测试执行环境前提。补充后的 22 条覆盖项为 §7 的测试用例设计和 §11 的覆盖分析提供了完整依据。
...
> Strategy 存储与 coverage_items 分离（`strategies.json`）。策略修订的 Before/After 证据保存在 `07_strategy_assignment_before.csv` 和 `07_strategy_assignment.csv`。
---

## 6. AutoTestDesign Prompt Design and Interactive Review（§6）

AutoTestDesign 使用 5 个 prompt 模板支持测试设计流程：`parse`（需求解析）、`risk`（风险分析）、`coverage`（覆盖项识别）、`testcase`（测试用例与 oracle 生成）和 `improve`（执行后的用例增广建议）。这些 prompt 均要求返回结构化 JSON，并限制字段 schema 与 oracle operator 集合（`eq/ne/gte/lte/gt/lt/contains`），以便后续脚本解析、审查和执行。
...
**Prompt 设计要点**：① 通过“仅返回 JSON”和固定 schema 保证结果可解析；② 通过字段白名单（v2/REV-015）消除不可验证字段；③ 通过 VCU 输出取值语义（v3/TOOL-FIX-007）约束 oracle 的合法取值；④ 通过 operator 受限集避免不可执行断言；⑤ 保留 `oracle_reasoning` 以支持人工审查。

### 6.2 testcase prompt 两轮迭代（Prompt Design 的核心证据）

testcase prompt 依据设计期审查和执行反馈进行了两轮改进。改进重点放在 prompt 约束与生成规则上，而不是对单条用例进行零散修补：
...
**分析**：主要 Before/After = v1（工具初版 94，含 21 条不可验证字段）↔ v4（最终 96，0 条不可验证字段、96/96 通过）。中间 v2/v3 完整保留，体现"两轮生成规则改进 + 两轮人工审查"的迭代闭环：测试用例并非一次性生成后直接采用，而是依据设计期审查和执行期反馈持续修订（详见 §6、§10.4、§12）。
...
> **追溯字段限制说明**：CSV 中 16 行的 `Coverage Item ID` 为占位串（如 `EP-REQ-002` / `N/A` / 空），系 v3 重生成时 UUID 链丢失所致；这些行仍可经 `(REQ + 技术)` 映射回 canonical CI，**不影响 22/22 覆盖结论**，已列为流程改进项（§13.3）。
...
- **工具执行链路**（TOOL-FIX-008）：前端「执行全部」→ `/api/execute` → `pytest_runner.py` subprocess 执行 pytest → 解析 JUnit XML 并按 case UUID 回映射 → 前端 Results 显示真实通过率。
- **命令**：
  ```
...
**分析**：负向用例占 49%，用于验证非法输入、边界条件和已知缺陷场景下的系统行为。负向用例 PASS 的含义是 VCU 按预期拒绝、报错、维持状态或复现缺陷，**而非返回 success**。例如 REQ-012 卡死用例 PASS = 成功复现缺陷；CP=8V 负向用例 PASS = VCU 正确拒绝唤醒并维持 state09。本轮无遗留失败（所有 oracle 偏差已在执行期审查修正，见 §10.4）。
...
→ **隔离结论**：当且仅当 **循环次数 ≥ 3 且相邻间隔 < 1.0s** 时触发卡死。两个边界均被覆盖：次数 2 不触发 / 3 触发（阈值=`rapid_cycle_threshold=3`）；间隔 0.9s 触发 / **1.0s 不触发** / 1.1s 不触发（阈值=`rapid_cycle_interval_s=1.0`，判定为"间隔≥1s 即重置计数"）。
...
```
RPN=1（最高优先级）正因其触发门槛低、影响整车可用性。该缺陷由 REQ-012 的设计用例稳定复现，是本文档中测试结果分析的关键证据。
...
AutoTestDesign 的 Optimize 页提供两项基于规则和集合覆盖的优化：
...
**最小化有效性分析（在 VCU 上执行验证）**：最小化后 65 条全部通过，需求覆盖、Coverage Item 覆盖和 DEF-001 检出能力均保留，执行耗时降低 55%。代价也被量化：分支覆盖减少 2 条分支弧（L199 快速循环计数复位、L239 电压达标但时序不足），因为 `(需求 × 技术 × 正负向)` 准则比分支级覆盖更粗，会合并同一覆盖单元内的边界细分变体。**建议用法**：最小化套件用于回归冒烟 / CI 快速门禁；完整验收仍执行全量 96 条。
...
**追溯链限制说明**：22 条 CI 中 21 条可由用例 `coverage_item_id`（UUID）直接 join；其余 1 条（#19 区的 REQ-012「active_dtcs 含 DTC_001」）在 v3 重生成时丢失 UUID 链，但功能上仍被 REQ-012 的 6 条卡死用例覆盖（每条都断言 `active_dtcs contains DTC_001`），故按 (req+技术) 映射仍为已覆盖；另有 16 条 v3 重生成用例 `coverage_item_id` 为占位串。**此为追溯字段一致性问题，不影响覆盖结论（22/22 成立）**，已列为流程改进项。
...
> **范围界定**：Assignment「Mainly」清单中 **#7 基于证据的改进**排在 **#6 结果分析**之后，本文将其解释为用例执行并获得执行/覆盖证据之后新增的有效用例。因此本节只收录第一轮执行后的有效新增（两类来源）；设计期改进（CI 14→22、prompt 迭代、不可验证字段 21→0，见 §4/§6）与执行纠正链（48→96，见 §10.4）按时序归各自板块。

### 12.1 来源 A — 第二轮 LLM 用例增广（`/api/improve` + 人工评审）

第一轮 96/96 执行完成后，将执行结果和覆盖信息输入 `improve` prompt（qwen3.7-max，67s），得到 **8 条**增广建议。测试人员随后对照 `simulator.py` 的实际行为逐条评审，最终采纳 **3 条**（采纳率 37.5%）：

| # | REQ | 建议 | 人工评审结论 |
|---|---|---|---|
| 3 | REQ-011 | 休眠条件全满足 + 同时来唤醒 | 采纳，修正 oracle：LLM 建议 state11，实际 `simulate()` 先判 `_has_sleep_inputs`，因此休眠优先，期望 state9 |
| 5 | REQ-004 | CC 与 CC2 同时满足唤醒 | 采纳，修正 oracle：LLM 建议 `reason in[4,5,6]`（操作符不支持），实际 CC 优先于 CC2，期望 reason 4 |
...
实测与**人工修正后**的 oracle 完全一致（state9 / reason4 / reason6），说明原始建议中的部分 oracle 需要人工校正。**有效性说明**：新增 3 条组合/优先级条件，但**分支覆盖 +0**（76.65%→76.65%）。这是因为优先级属于判定顺序行为，执行路径已在基线用例中覆盖，分支覆盖对该类行为不敏感。
...
### 12.3 结论
§12 收录 **2 类来源、4 条执行后新增有效用例（4/4 通过）**：① **LLM 增广**（3 条）采纳率 37.5%，新增组合/优先级用例；5 条超出范围或不可执行的建议被拒绝，2 条 oracle 被修正，分支覆盖 +0；② **覆盖缺口驱动**（1 条 L273）可量化地将纯 Module A 分支覆盖 **89.0%→90.2%**。
...
**改进建议**：① 关闭 reasoning（`LLM_ENABLE_THINKING=false`，已部分缓解）；② 换本地/小模型做结构化生成；③ 相同 prompt 结果缓存；④ 14 条需求并发生成（当前串行，墙钟从 Σ 降到 max）；⑤ 流式输出 + 非交互场景离线预生成。
...
### 13.2 SUT 限制与残余风险
- **SUT 限制**：SIL 级仿真，无真实 HIL/dSPACE/CAN 物理层/1kHz 采样；本 Artifact **仅测 Module A**，Module B/C/D/E（过压/欠压保护、CAN 错误、DTC 清除、功耗告警）不属于本文测试范围（见 §1.3）。
